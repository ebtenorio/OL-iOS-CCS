...
{
	public class ProductListingsSource : UITableViewSource
	{
		...
		void PopulateProductDictionary ()
		{
			foreach (var group in ProductGroupList) {
				if (ProviderID > 0) {
					var productList = dalFacade.getProductListByProductGroupAndProviderID (ProviderID, group.Name);
					if (productList != null)
		...
		public void RefreshData ()
		{
			...
			if (searchString == null || searchString.Trim ().Length <= 0) {
				...
				case SearchScope.PRODUCT_NAME:
					if (ProviderID > 0)
						ProductGroupList = dalFacade.getProductGroupsByProductName (ProviderID, searchString);
					else
						ProductGroupList = dalFacade.getProductGroupsByProductName (searchString);
					foreach (var group in ProductGroupList) {
						ProductDictionary.Add (group.Name, dalFacade.getGroupedProductsByProductName (ProviderID, group.Name, searchString));
					}
					break;
				case SearchScope.PRODUCT_CODE:
					if (ProviderID > 0)
						ProductGroupList = dalFacade.getProductGroupsByProductCode (ProviderID, searchString);
					else 
						ProductGroupList = dalFacade.getProductGroupsByProductCode (searchString);
					foreach (var group in ProductGroupList) {
						ProductDictionary.Add (group.Name, dalFacade.getGroupedProductsByProductCode (ProviderID, group.Name, searchString));
					}
					break;
				case SearchScope.PRODUCT_GROUP:
					if (ProviderID > 0)
						ProductGroupList = dalFacade.getProductGroupsByName (ProviderID, searchString);
					else
		...
		{
			return ProductGroupList[section].Name;
		}
